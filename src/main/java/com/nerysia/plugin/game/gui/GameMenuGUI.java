...
public class GameMenuGUI {
    ...
    public GameMenuGUI(Nerysia plugin, GameType gameType) {
        this.plugin = plugin;
        this.gameType = gameType;
    }
    
    public void open(Player player) {
        Inventory inv = Bukkit.createInventory(null, 54, gameType.getDisplayName() + " §8- §7Parties");
        ...
        createLore.add("§7une nouvelle partie");
        createMeta.setLore(createLore);
        createButton.setItemMeta(createMeta);
        inv.setItem(49, createButton);
        
        // Afficher les parties disponibles
        List<Game> games = plugin.getGameManager().getGamesByType(gameType);
        int slot = 0;
    ...
    private ItemStack createGameItem(Game game) {
        ...
        meta.setDisplayName(game.getMode().getDisplayName() + " §8- §f" + game.getName());
