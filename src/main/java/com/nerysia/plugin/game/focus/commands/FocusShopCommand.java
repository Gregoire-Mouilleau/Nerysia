...
/**
 * Commande /fshop pour ouvrir le shop Focus
 */
public class FocusShopCommand implements CommandExecutor {
    
    private final Nerysia plugin;
    
    public FocusShopCommand(Nerysia plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (controller.getState() != FocusGameController.State.LOBBY) {
            player.sendMessage(ChatColor.RED + "Le shop est uniquement accessible dans le lobby !");
            return true;
        }
        
        plugin.getFocusShopGUI().open(player, null);
        return true;
    }
}
